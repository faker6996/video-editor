...
export const DatePicker: React.FC<DatePickerProps> = ({
  ...
  const datePickerContent = isOpen && dropdownPosition ? (
    <div
      data-datepicker
      style={{
        position: 'absolute',
  ...
  return (
    <>
      <button
        ...
        className={cn(
          cn('flex w-full items-center justify-between rounded-md border border-input bg-background', size === 'sm' ? 'px-2.5 py-1.5 text-xs h-9' : 'px-3 py-2 text-sm h-10'),
...
export const DateRangePicker: React.FC<{
  ...
  const panel = isOpen && dropdownPosition ? (
    <div
      style={{ position: 'absolute', top: dropdownPosition.top, left: dropdownPosition.left, width: dropdownPosition.width || 256, zIndex: 9999 }}
      data-state={isOpen ? 'open' : 'closed'}
      className={cn('z-[9999]', 'data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95', 'data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95')}
    >
      <div ref={panelRef} className={cn('rounded-md border bg-popover text-popover-foreground shadow-md', 'backdrop-blur-sm bg-popover/95 border-border/60 p-4 w-64')}>
        <div className="flex items-center justify-between mb-3">
          <Button variant="ghost" size="sm" onClick={() => setViewDate(new Date(viewDate.getFullYear(), viewDate.getMonth() - 1, 1))} className="p-1 h-auto">
            <ChevronLeft className="h-4 w-4" />
          </Button>
  ...
  const label = tempStart && tempEnd
    ? `${displayFormat(tempStart)} - ${displayFormat(tempEnd)}`
    : tempStart
    ? `${displayFormat(tempStart)} - ...`
    : placeholder;
