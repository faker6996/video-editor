-- Migration: Create text_regions table for PostgreSQL (Safe version)
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ocr_results') THEN
        ALTER TABLE text_regions ADD CONSTRAINT fk_text_regions_ocr_result_id 
...
DROP INDEX IF EXISTS idx_text_regions_ocr_result_id;
DROP INDEX IF EXISTS idx_text_regions_region_index;
DROP INDEX IF EXISTS idx_text_regions_confidence;
DROP INDEX IF EXISTS idx_text_regions_is_edited;
